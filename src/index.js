...
class dateObject {
  ...
  get ordinal() {
    let stringDate = this._date.getDate().toString();
    let lastChar = stringDate.slice(-1);
    switch (lastChar) {
      case "1":
        stringDate += "st";
        break;
  ...
  /**
   * This function formats a date using a mask.
   * @param {String} input format mask
   * @returns {String} formatted string of dateObject
   */
  format = (mask) => {
    const formatObject = {
      Y: this.year,
  ...
  /**
   * This function returns a string indicating how far away a date is from now. 
   * @returns {String} string describing how far in the past or future a dateObject is from now. 
   */
  when = () => {
    let past = false;
    let date = this._date;
